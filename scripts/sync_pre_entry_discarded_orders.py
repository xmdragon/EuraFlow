...
"""
临时脚本：同步录单前废弃订单
从跨境84平台拉取录单前废弃的订单，更新本地状态为"已取消"

API接口：POST https://www.kuajing84.com/index/Accountorder/order_list_purchase
参数：page={page}&limit=50&is_discard=0&is_save_search=11

使用方式：
    python3 scripts/sync_pre_entry_discarded_orders.py
"""
import asyncio
import sys
...
# 跨境84 API配置
KUAJING84_API = "https://www.kuajing84.com/index/Accountorder/order_list_purchase"
...
async def update_local_orders(orders):
    ...
    async with db_manager.get_session() as db:
        for order in orders:
            ...
            if not order_number:
                logger.warning(f"⚠️  订单缺少 order_number，跳过")
                continue
...
async def main():
    ...
    logger.info(f"\n📦 从跨境84获取到 {len(orders)} 条录单前废弃订单")

    # 2. 更新本地订单
    updated_count, not_found_count = await update_local_orders(orders)
    ...
    logger.info("=" * 60)
    logger.info(f"📊 统计信息：")
    logger.info(f"   - 跨境84废弃订单：{len(orders)} 条")
