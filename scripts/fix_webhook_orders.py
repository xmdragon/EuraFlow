...
async def sync_webhook_orders():
    ...
    async with db_manager.get_session() as session:
        # 查找所有 webhook 临时订单及其关联的 posting
        stmt = select(OzonOrder, OzonPosting).join(
            OzonPosting, OzonOrder.id == OzonPosting.order_id
        ).where(
            OzonOrder.ozon_order_id.like('webhook_%')
        ).order_by(OzonOrder.id)

        result = await session.execute(stmt)
        webhook_orders = result.all()

        total = len(webhook_orders)
        ...
        for idx, (order, posting) in enumerate(webhook_orders, 1):
            ...
            try:
                # 获取店铺信息
                shop = await session.get(OzonShop, shop_id)
                if not shop:
