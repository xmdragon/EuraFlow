...
async def get_shops(
    ...
    try:
        ...
        result = await db.execute(
            select(OzonShop).where(OzonShop.owner_user_id == owner_user_id)
        )
        ...
        for shop in shops:
            ...
            product_stats_result = await db.execute(
                select(
                    func.count(OzonProduct.id).filter(
                        OzonProduct.status == 'active'
                    ).label('active_count'),
                    func.count(OzonProduct.id).filter(
                        OzonProduct.status == 'inactive'
                    ).label('inactive_count'),
                    func.count(OzonProduct.id).filter(
                        OzonProduct.status == 'out_of_stock'
                    ).label('out_of_stock_count'),
                    func.count(OzonProduct.id).filter(
                        OzonProduct.is_archived == True
                    ).label('archived_count'),
                    func.count(OzonProduct.id).filter(
                        OzonProduct.visibility == True
                    ).label('visible_count')
            ...
            order_count_result = await db.execute(
                select(func.count(OzonOrder.id)).where(OzonOrder.shop_id == shop.id)
            )
...
async def sync_shop(
    ...
    try:
        ...
        return {
            "ok": True,
            "task_id": task_id,
            "message": f"同步任务已启动 (类型: {sync_type}, 全量: {full_sync})"
        }
