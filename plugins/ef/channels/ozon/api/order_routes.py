...
async def sync_single_order(
    ...
    try:
        ...
        # 4. 使用订单同步服务处理数据
        print(f"[同步调试-PRINT] 开始处理posting数据: posting_number={posting_number}, products_count={len(posting_data.get('products', []))}")
        ...
        print(f"[同步调试-PRINT] _process_single_posting完成，准备commit")
        logger.info(f"[同步调试] _process_single_posting完成，准备commit")
        await db.commit()
        print(f"[同步调试-PRINT] commit完成")
