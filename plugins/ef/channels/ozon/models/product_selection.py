...
class ProductSelectionItem(Base):
    ...
    def to_dict(self):
        ...
        return {
            ...
            'rfbs_commission_high': float(self.rfbs_commission_high) if self.rfbs_commission_high else None,
            ...
            'fbp_commission_high': float(self.fbp_commission_high) if self.fbp_commission_high else None,
            # 销售数据
            'monthly_sales_volume': self.monthly_sales_volume,
            'monthly_sales_revenue': float(self.monthly_sales_revenue) if self.monthly_sales_revenue else None,
            'daily_sales_volume': float(self.daily_sales_volume) if self.daily_sales_volume else None,
            'daily_sales_revenue': float(self.daily_sales_revenue) if self.daily_sales_revenue else None,
            'sales_dynamic_percent': float(self.sales_dynamic_percent) if self.sales_dynamic_percent else None,
            'conversion_rate': float(self.conversion_rate) if self.conversion_rate else None,
            # 物流信息
            'package_weight': self.package_weight,
            'package_volume': float(self.package_volume) if self.package_volume else None,
            ...
            'availability_percent': float(self.availability_percent) if self.availability_percent else None,
            'ad_cost_share': float(self.ad_cost_share) if self.ad_cost_share else None,
            # 竞争对手数据
            ...
            'card_add_to_cart_rate': float(self.card_add_to_cart_rate) if self.card_add_to_cart_rate else None,
            'search_views': self.search_views,
            'search_add_to_cart_rate': float(self.search_add_to_cart_rate) if self.search_add_to_cart_rate else None,
            'click_through_rate': float(self.click_through_rate) if self.click_through_rate else None,
            'promo_days': self.promo_days,
            'promo_discount_percent': float(self.promo_discount_percent) if self.promo_discount_percent else None,
            'promo_conversion_rate': float(self.promo_conversion_rate) if self.promo_conversion_rate else None,
            'paid_promo_days': self.paid_promo_days,
            'return_cancel_rate': float(self.return_cancel_rate) if self.return_cancel_rate else None,
            # 基础字段（上品帮）
